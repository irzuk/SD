package org.Roguelike.collections.map;

public class MapElementsParameters {
    public static final int CELL_BORDER = 5;
    public static final int MAP_WIDTH = CELL_BORDER * 100;
    public static final int MAP_HEIGHT = CELL_BORDER * 60;
    public static final int HERO_WIDTH = CELL_BORDER;
    public static final int HERO_HEIGHT = CELL_BORDER;
    public static final int ENEMY_WIDTH = CELL_BORDER;
    public static final int ENEMY_HEIGHT = CELL_BORDER;
    public static final int DOOR_VERTICAL_WIDTH = CELL_BORDER;
    public static final int DOOR_VERTICAL_HEIGHT = CELL_BORDER;
    public static final int DOOR_HORIZONTAL_WIDTH = CELL_BORDER;
    public static final int DOOR_HORIZONTAL_HEIGHT = CELL_BORDER;
    public static final int CHEST_WIDTH = CELL_BORDER;
    public static final int CHEST_HEIGHT = CELL_BORDER;
}
